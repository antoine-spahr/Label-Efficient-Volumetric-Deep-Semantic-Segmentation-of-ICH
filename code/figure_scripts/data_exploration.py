...
# Load data
########################################################################################################################
DATA_PATH = '../../data/publicSegICH2D/'

diagnos_df = pd.read_csv(DATA_PATH + 'ct_info.csv', index_col=0)
patient_df = pd.read_csv(DATA_PATH + 'patient_info.csv', index_col=0)
...
print(f'>>> Number of ICH-positive CT : {len(diagnos_df[diagnos_df.Hemorrhage == 1])}')
...
ax1.hist(patient_df['Age'], color=color, bins=80//5, range=(0,80))
ax1.hist(patient_df['Age'], histtype='step', color='black', bins=80//5, range=(0,80), linewidth=1)
...
fig.savefig('../../figures/publicSeg_data_exploration/metadata_stat.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
diagnos_df['No_Hemorrhage'] = 1 - diagnos_df.Hemorrhage # make a columns positive for ICH
...
fig.savefig('../../figures/publicSeg_data_exploration/data_stats.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
CT_file_df['filename'] = CT_file_df.apply(lambda row: f'{row.PatientNumber:03}/{row.SliceNumber}.tif', axis=1)
CT_file_df['mask_filename'] = CT_file_df.apply(lambda row: 'None' if row.Hemorrhage == 0 else f'{row.PatientNumber:03}/{row.SliceNumber}_ICH_Seg.bmp', axis=1)

fig, axs = plt.subplots(4, 6, figsize=(16, 10), gridspec_kw=dict(hspace=0.0, wspace=0.2))

no_ICH_list = CT_file_df[CT_file_df.Hemorrhage == 0].filename.sample(n=12, random_state=1).values
ICH_list = CT_file_df[CT_file_df.Hemorrhage == 1].filename.sample(n=12, random_state=69).values
ICH_mask_list = CT_file_df[CT_file_df.Hemorrhage == 1].mask_filename.sample(n=12, random_state=69).values

for ax, fn in zip(axs[:,:3].reshape(-1), no_ICH_list):
    ax.imshow(io.imread(DATA_PATH + 'Patient_CT/' + fn), cmap='gray')
    ax.set_axis_off()

for ax, fn, mask_fn in zip(axs[:,3:].reshape(-1), ICH_list, ICH_mask_list):
    ax.imshow(io.imread(DATA_PATH + 'Patient_CT/' + fn), cmap='gray')
    mask = io.imread(DATA_PATH + 'Patient_CT/' + mask_fn).astype('bool')
    ax.imshow(np.ma.masked_where(mask == False, mask), cmap=matplotlib.colors.ListedColormap(['xkcd:vermillion']), alpha=.8)
    ax.set_axis_off()
...
fig.savefig('../../figures/publicSeg_data_exploration/CT_sample.pdf', dpi=300, bbox_inches='tight')
plt.show()
...
PatientID = ICH_patient_list[10]
CT_list, mask_list = [], []
for _, row in CT_file_df[CT_file_df.PatientNumber == PatientID].iterrows():
    CT_list.append(io.imread(DATA_PATH + 'Patient_CT/' + row.filename))
    if row.mask_filename == 'None':
        mask_list.append(np.zeros_like(CT_list[-1]))
    else:
        mask_list.append(skimage.img_as_bool(io.imread(DATA_PATH + 'Patient_CT/' + row.mask_filename)))

pred2GIF(CT_list, mask_list, f'../../figures/publicSeg_data_exploration/{PatientID}_CT.gif', fps=4)
