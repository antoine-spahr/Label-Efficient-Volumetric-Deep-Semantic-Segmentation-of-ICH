...
from src.models.optim.LossFunctions import InfoNCELoss
from src.utils.print_utils import print_progessbar

class ContrastiveEncoder:
    """
    Class defining an encoder trainable on the contrastive task using the batch as comparison set (similarly as Chen et
    al. 2019).
    """
    def __init__(self, net, n_epoch=150, batch_size=32, lr=1e-3, lr_scheduler=optim.lr_scheduler.ExponentialLR,
                 lr_scheduler_kwargs=dict(gamma=0.95), loss_fn=InfoNCELoss, loss_fn_kwargs=dict(32, tau=0.1, device='cuda'),
                 weight_decay=1e-6, num_workers=0, device='cuda', print_progress=False):
        """
        Build a Contrastive model for an Encoder.
        ----------
        INPUT
        ...
        OUTPUT
            |---- ContrastiveEncoder () the model for the Contrastive learning task of encoder.
        """
        self.net = net
    ...
    def train(self, dataset, checkpoint_path=None):
        """
        Train the passed encoder network on the given dataset for the contrastive task.
        ----------
        INPUT
        ...
        try:
            ...
            epoch_loss_list = []
        # Train Loop
        logger.info('Start trianing the Encoder on the contrastive task.')
        ...
        logger.info(f"Finished training on the encoder on the contrastive task in {timedelta(seconds=int(self.outputs['train']['time']))}")

    def evaluate(self, dataset):
        """
        Evaluate the passed network on the given dataset for the Contrastive task (get t-SNE representation of samples).
        ----------
        INPUT
        ...
        # Evaluate
        logger.info('Start Evaluating the contrastive encoder model.')
        start_time = time.time()
        ...
        with torch.no_grad():
            for b, data in enumerate(loader):
                ...
                idx_repr += list(zip(idx.cpu().data.tolist(), z.cpu().data.tolist()))
                # print_progress
                if self.print_progress:
        ...
        logger.info(f"Finished evaluating of encoder on the contrastive task in {timedelta(seconds=int(self.outputs['eval']['time']))}")
